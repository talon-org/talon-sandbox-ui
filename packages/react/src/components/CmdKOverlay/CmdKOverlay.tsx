...
export function CmdKOverlay({
  ...
  useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === 'ArrowDown') {
        e.preventDefault();
        setActiveIdx((a) => Math.min(filtered.length - 1, a + 1));
      }
      if (e.key === 'ArrowUp') {
        e.preventDefault();
    ...
    <div className="tln-cmdk-back" onClick={onClose} role="presentation">
      ...
      >
        <div className="tln-cmdk-search">
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" style={{ color: 'var(--fg-3)', flexShrink: 0 }}>
        ...
        <div className="tln-cmdk-list" role="listbox" aria-label="Results">
          {groups.length === 0 && (
          ...
          {groups.map((g) => (
            <div key={g.name} role="group" aria-label={g.name}>
              ...
              {g.items.map((it) => (
                <div
                  key={it.name + it.idx}
                  className={cx('tln-cmdk-item', it.idx === activeIdx && 'tln-cmdk-item--active')}
                  role="option"
                  aria-selected={it.idx === activeIdx}
